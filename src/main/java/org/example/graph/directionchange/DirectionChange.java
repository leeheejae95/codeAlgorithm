package org.example.graph.directionchange;
...
public class DirectionChange {
    public int solution(int[][] board) {
        int answer = 0;
        // 1~4번 방향대로 나열하기( 같은 방향으로 나열 )
        int[] dx = {0, 0, 1, -1};
        int[] dy = {1, -1, 0, 0};
        int n = board.length;
        int m = board[0].length;
        int[][] cost = new int[n][m]; // 방향을 바꾼 최소 갯수
        for(int i = 0; i < n; i++) Arrays.fill(cost[i], Integer.MAX_VALUE);
        cost[0][0] = 0;
        PriorityQueue<int[]> pq = new PriorityQueue<>((a, b) -> a[2] - b[2]); // 방향을 바꾼 횟수가 작은 순으로 정렬
        pq.add(new int[]{0, 0, 0});
        while(!pq.isEmpty()) {
            int[] cur = pq.poll();
            int dir = board[cur[0]][cur[1]] - 1; // 방향 배열은 0부터 시작이라 같게 해주려면 board배열에 -1을 해줘야 한다
            ...
            for(int k = 0; k < 4; k++) {
                ...
                if(k == dir && cost[nx][ny] > cur[2]) {
                    ...
                    if (cost[nx][ny] > cur[2] + 1) { // 방향이 바뀌면 +1해주기
                        cost[nx][ny] = cur[2] + 1;
...
//        return answer;
    ...
    public static void main(String[] args){
        DirectionChange T = new DirectionChange();
